...
//引入尾部
$('#foot').load("../../src/html/footer.html"); //默认是表单1，点击用户密码登录，显示表单2
...
$('#name1').blur(function () {
  //获取输入的用户手机号
  var formPhone = $('#name1').val(); //获取cookie

  var formStr = $.cookie("form") ? $.cookie("form") : '';
  var formObj = convertStrToObj(formStr); //判断手机号是否在cookie中
...
$('#pass').blur(function () {
  ...
  var formPhone = $('#name2').val();
  var formStr = $.cookie('form') ? $.cookie('form') : '';
  var formObj = convertStrToObj(formStr); //如果账号正确再进行密码判断
...
$('#registerNow2').click(function () {
  ...
  var formBlock2 = $('#passSpan').css('display'); //提示错误框不在，判断可以登录
