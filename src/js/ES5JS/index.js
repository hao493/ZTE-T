...
//引入头尾部
$('#head').load("header.html"); //引入尾部

$('#foot').load("footer.html"); //banner图
...
var liWidth = $ul_li[0].offsetWidth; //简易运动

function sport(obj, target) {
  clearInterval(obj.timer);
  obj.timer = setInterval(function () {
    var cur = obj.offsetLeft;
    var speed = target > cur ? 100 : -100;
...
function autoPlay() {
  ...
  for (var i = 0, len = $ul2_li.length; i < len; i++) {
    $ul2_li[i].className = '';
  }
...
$titR1.hover(function () {
  ...
  ajax('../json/accMain1.json', function (data) {
    ...
    for (var j = 0, len = arr.length; j < len; j++) {
      ...
      li.innerHTML = "<a href=\"#\">\n\t\t\t\t\t<span><img src=\"".concat(arr[j].img, "\"></span>\n\t\t\t\t\t<span>").concat(arr[j].tit, "</span>\n\t\t\t\t\t<span>").concat(arr[j].money, "</span>\n\t\t\t\t</a>");
