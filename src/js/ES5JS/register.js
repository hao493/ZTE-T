...
$(function () {
  $message = $('#message');
  $getMessage = $('#getMessage'); //name框失焦事件
  ...
  $('#registerNow').click(function () {
    ...
    console.log($('#password').val()); //获取cookie中字符串

    var formStr = $.cookie("form") ? $.cookie("form") : ""; //获取当前name可输入框中的值，手机号

    var formPhone = $('#name').val(); //获取当前密码框中的值

    var formPass = $('#password').val(); //将cookie字符串转成对象

    var formObj = convertStrToObj(formStr); //判断cookie中手机号是否注册过，即是否已经存在
    ...
    if (formPhone in formObj) {
      ...
      var value3 = $('#againPass').val(); //判断错误提示框不存在，且内容框的值不为空
...
function convertStrToObj(formStr) {
  ...
  var goods = formStr.split(":"); //空对象存分割后字符串用

  var obj = {};

  for (var i = 0; i < goods.length; i++) {
    var data = goods[i].split(","); //以商品的id为健，商品的其他信息为值，这个值也设计为一个对象
