...
function ajax(url, fnWin, fnFaild) {
  ...
  var xhr = window.XMLHttpRequest ? new XMLHttpRequest() : new ActiveXObject("Microsoft.XMLHTTP"); //2. 拨号-------与服务器建立连接

  xhr.open('GET', url, true); //3. 说话 ----- 发送请求

  xhr.send(); //4. 听并等待接收 --- 利用监听事件，将请求的数据通过回调函数返回
