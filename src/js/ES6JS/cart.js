...
$(function(){
    //取出cookie中的信息
    var vartStr = $.cookie('cart') ? $.cookie('cart') :'';
    //如果是空
    if(!cartStr){
        ...
        //遍历所有的商品生成html添加到购物车列表
        for(var id in cartObj){
            //id数及品种类数，cartObj中的子对象数
            //商品信息对象，子对象
            var good = cartObj[id];
            var str = `<div id="product" data-good-id="${good.id}">
            <div id="proSelBox">
                <input type="checkbox" name="proSel" id="proSel" value="" />
            </div>
            <div id="proTit">
                <a href="#">${good.img}</a>
                <span>${good.color}/${good.system}/${good.versions}</span>
            </div>
            <div id="proMoney">
            ...
            <div id="proNum">
                <form>
                    <button class="sub">-</button><input type="text" name="" id="txt" value="${good.num}" data-now=""/><button class="add">+</button>
                </form>
            </div>
            <div id="proMoneyAll">
                <span>${good.moneyAll}</span>
            </div>
            <div id="delete">
        ...
        $('#proNum.add').click(function(){
            ...
            cartObj[id].num += 1;
            //将页面上显示的数量+1
            $(this).sinlings('input').val('' + cartObj[id].num);
            //更新小计
            $(this).parent().sinlings('#proMoneyAll').html(cartObj[id].num * cartObj[id].money + '');
            //将信息放回cookie
        ...
        $('#proNum .sub').click(function(){
            ...
            if(cartObj[id].num > 1){
                ...
                cartObj[id].num -= 1;
                //将页面上显示的数量-1
                $(this).sibling('input').val('' + cartObj[id].num);
                //更新
                $(this).parent().sinlings('#proMoneyAll').html(cartObj[id].num * cartObj[id].money + '');
                //将信息返回cookie
                $.cookie('cart',convertObjToCartStr(cartObj),{
        ...
        $('#proNum input').blur(function(){
            ...
            $(this).sinlings('input').val('' + cartObj[id].num);
            //更新小计
            $(this).parent().siblings('#proMoneyAll').html(cartObj[id].num * cartObj[id].money + '');
            //把信息返回到cookie
            $.cookie('cart',convertObjToCartStr(cartObj),{
                expires:7,
                path:'/'
...
window.onload = function(){
    ...
    //更新购物车中商品总数
    $('#monChoNum').html(allNum);
    //商品所选中的所有价格
    $('#moenyChooseAll span').html(allMon);
    //商品总数
    $('#headNum').html(allNum);
}
